class ApplicationController < ActionController::Base
  ...
  def my_archive_to_cookie(my_ids=[])
  	unless my_ids.nil? || my_ids.empty?
    	cookies.permanent[:my_archive] = { :value => my_ids.join(","), :key => "_session_id", :domain => COOKIE_DOMAIN }
    else
    	cookies.permanent[:my_archive] = { :value => "", :key => "_session_id", :domain => COOKIE_DOMAIN }
    end
  ...
  def admin_required
    authenticate_or_request_with_http_basic do |user_name, password|
      user_name == 'Editor2011' && password == 'nat5ees4ok5ots8'
