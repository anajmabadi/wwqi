class Admin::CollectionsController < Admin::AdminController
	...
	def export
		begin
			...
			flash[:error] = "There was a problem finding collections: " + error.message
		@error = true
		end

		respond_to do |format|
			format.html { redirect_to admin_collections_path, :error => flash[:error] }
			format.csv do
				csv_string = make_custom_csv(@collections)
				# send it to the browsah
				send_data csv_string,
                :type => 'text/csv; charset=utf-8; header=present',
                :disposition => "attachment; filename=collections.csv"
			end
			format.xml  { render :xml => @collections }
		end
